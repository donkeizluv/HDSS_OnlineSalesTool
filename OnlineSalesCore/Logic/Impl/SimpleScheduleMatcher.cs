...
namespace OnlineSalesCore.Logic.Impl
{
    public class SimpleScheduleMatcher : IScheduleMatcher
    {
        ...
        public async Task<(bool, List<AppUser>, string)> GetUserMatchedSchedule(string posCode, DateTime date)
        {
            ...
            var schedule = _context.PosSchedule
            ...
            //Match specific time
            var matched = schedule.Where(s => s.Shift.ShiftDetail
                .Any(d => (timeOfDate >= d.StartAt && timeOfDate <= d.EndAt) && s.User.Active));
            if (!matched.Any())
            {
            ...
            return (true, await matched.Select(m => m.User).Distinct().ToListAsync(), string.Empty);
        }
    }
}
