...
namespace OnlineSalesCore.Services
{
    public class UserService : ContextAwareService, IUserService
    {
        ...
        public async Task<IEnumerable<AppUserDTO>> SuggestAssign(string q)
        {
            var baseQuery = DbContext.AppUser
                .Where(u => (u.Username.Contains(q) || u.Hr.Contains(q))
                    && u.Role.Name == RoleEnum.CA.ToString()
                    && u.Active);
            switch (Role)
            {
                case RoleEnum.CA:
                    throw new BussinessException("CA cant use this feature");
                case RoleEnum.BDS:
                    //Return CA of this BDS
                    baseQuery = baseQuery.Where(u => u.ManagerId == UserId);
                    break;
                case RoleEnum.ASM:
