...
namespace OnlineSalesCore.Services
{
    public class ScheduleService : ContextAwareService, IScheduleService
    {
        ...
        private readonly ShiftScheduleOptions _options;
        ...
        public async Task<ShiftAssignerVM> Get()
        {
            ...
            switch (Role)
            {
                case Const.RoleEnum.CA:
                    return await CreateVM_CA(now);
                case Const.RoleEnum.BDS:
                    return await CreateVM_BDS(now);
        ...
        public async Task<int> Create(ScheduleContainer schedule)
        {
            ...
            var now = DateTime.Now;
            if(!AllowCreate(now)) throw new BussinessException("Not accepting new schedule at the moment.");
        ...
        //Accept new schedule according to config
        public bool AllowCreate(DateTime current)
        {
            return current.Day >= _options.Start && current.Day <= _options.End;
        }
    }
}
