...
export default {
    ...
    computed: {
        ...
        canSaveSchedule() {
            ...
            return this.currentDays.every(d =>
                d.Shifts.every(s => {
    ...
    methods: {
        ...
        createNewSchedule() {
            ...
            let emptyDays = this.createEmptyDays(
                ...
                for (let d of emptyDays) {
                    ...
                    if (loaded) {
                        ...
                        emptyShifts.forEach(rs => {
                            //Find shift
                            let loadedShift = loaded.Shifts.find(
                                l => l.ShiftId == rs.ShiftId
                            );
                            if (loadedShift) {
                                //If user still under mangement then re-use assign values
                                if (
                                    flattenUsers.includes(
                                        loadedShift.Assign.value
                                    )
                                ) {
        ...
        saveSchedule: async function() {
            ...
            let postObject = {
                ...
                targetMonthYear: format(
                    setDate(this.systemMonthYear, 1),
            ...
            try {
                ...
                this.$emit(
                    "showinfo",
                    "Có lỗi trong quá trình tạo lịch làm việc."
                );
            }
        },
        //Private
        //Transform to linear shift detail format
        toLinearShiftDetail(days) {
        ...
        //Internal use
        createEmptyDays(dayCount, shifts) {
            let arr = Array(dayCount)
                .fill(1)
                .map((x, y) => x + y);
            let days = arr.map(d => {
                return {
        ...
        reloadPrevSchedule: async function() {
            ...
            let scheduleContainer = this.currentPrevSchedules.find(
                s => s.PosScheduleId === this.selectedPrevSchedule
            );
            if (!scheduleContainer)
                throw "Cant get scheduleContainer: " +
                    this.selectedPrevSchedule;
            //Load & attach detail if not available
            if (!scheduleContainer.hasOwnProperty("details")) {
                scheduleContainer.details = await this.loadScheduleDetails(
                    scheduleContainer.PosScheduleId
                );
            }
            ...
            let emptyDays = this.createEmptyDays(
                scheduleContainer.TotalDaysOfMonth,
                containerShifts
            );
            //Transform to displaying days
            //Map linear schedule details to empty day objects
            emptyDays.forEach(emptyDay => {
                let containerShifts = scheduleContainer.details.filter(
                    shift => shift.Day === emptyDay.Day
                );
                //console.log(containerShifts);
                containerShifts.forEach(cs => {
                    let shift = emptyDay.Shifts.find(
                        shift => shift.ShiftId === cs.Shift.ShiftId
                    );
                    shift.Assign = {
                        value: cs.User.UserId,
                        label: cs.User.DisplayName
                    };
                });
            });
            //console.log(days);
            ...
            this.showModeText(
                "Xem ca trực: ",
                this.currentPOS,
                scheduleContainer.DisplayMonthYear
            );
        ...
        showModeText(mode, pos, monthYear) {
        ...
        composePOSLabel(pos) {
            return (
                pos.PosCode + (pos.HasCurrentMonthSchedule ? "" : " (chưa xếp)")
            );
            //Too long
